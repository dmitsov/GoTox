...
func (dht *DHT) friendIndex(publicKey []byte) (int) {
	...
	for i := 0; i < len(dht.friendList); i++ {
		if idEqual(dht.friendList[i].publickKey, publickKey) {
			return i	
		}	
	}
...
//
func (dht *DHT) friendList(friendIndex int) ([]net.UDPAddr) 
 	var friend *DHT_Friend = &dht.friendList[friendIndex]
...
func (dht *DHT) routeOneToFriend(friendId, packet []byte) (error) {
	...
	int listIndex = rand.Int() % len(ipList)
...
func (ipPortList []*net.UDPAddr) getIpPorts(ip net.IP) ([]uint16) {
	ports := make([]uint16, 0)
